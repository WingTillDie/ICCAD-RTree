 ...
 */
static int _RTreeInsertRect(RTREEMBR *rc, int tid,  RTREENODE *node, RTREENODE **new_node, int height)
{
    int i;
    RTREEBRANCH b;
    RTREENODE *n2;
...
#pragma warning( disable : 4311 )
            for (i = 0; i < MAXKIDS(tmp_nptr); i++)
            {
                ...
                {
                    RTreeInsertRect(&(tmp_nptr->branch[i].mbr), (int)tmp_nptr->branch[i].child, root, tmp_nptr->height);
                }
            }
#pragma warning(pop)
